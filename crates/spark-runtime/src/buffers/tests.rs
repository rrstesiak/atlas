...
fn test_buffer_arena_alloc() {
    ...
    // The table below enumerates every arena buffer and then checks
    // `alloc_count() == allocated.len()`, so the count is derived rather than
    // hardcoded (it subsumes the qwen4_exp branch's hand-tallied 31, which
    // included hc_lowrank_scratch + qsa_select_scratch — both listed below).
